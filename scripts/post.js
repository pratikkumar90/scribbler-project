function editText() {
  ...
  console.log("edit button=" + editButton.innerHTML);
...
function saveText() {
  ...
  saveButton.innerHTML =
    '<button class="btn btn-danger saved btn-common" id="editPost" onclick="saveText()">Edit <i class="fa fa-edit"></i></button>';
}
...
function saveComment() {
  ...
  console.log("arr=" + commentArray);
  console.log("length=" + commentArray.length);
  ...
  for (var i = commentArray.length - 1; i >= 0; i--) {
    if (i != undefined) {
      console.log("value=" + commentArray[i]);
      newCommentArea +=
